    ...
    if readfile and writefile then
        ...
        local success_file, error_file = pcall(function()
            ...
            if is_key_present == true then
                ...
                if onl_key then
                    text_box.Text = "Saved key is correct!"
                    game:GetService("TweenService"):Create(text_box, TweenInfo.new(0.2, Enum.EasingStyle.Quad), { TextColor3 = Color3.fromRGB(0, 255, 0) }):Play()
                    task.wait(.15)
                    ...
                    CloseGUI()
                    wait(1)
                    ourScript(false, true, "RimmIsCuteGirl!")
                else
    ...
    check_key.MouseButton1Click:Connect(function()
        ...
        if keyValid then
			...
			if KeyClass.is_banned then 
                text_box.Text = "You are banned!"
            else 
                text_box.Text = "Key invalid/expired!" 
            end
